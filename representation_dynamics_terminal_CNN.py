...
def run_dynamic_analysis():
    ...
    measure_option = sys.argv[4].split('=')[1]
    optimization = sys.argv[5].split('=')[1]
    ...
    config = {'dir_path': '/path/to/working/dir', 'data': data_option, 'model': model_option,
              ...
              'layer_num': layer_num, 'input_size': 100, 'optimization': optimization, 'feature_option': 'block'}
    ...
    terminal_layer_path = config['dir_path'] + '/figures/' + config['data'] + '_' + config['model'] + '_' \
                          + config['measure'] \
                          + '_' + config['optimization'] + '_' + str(config['lr']) + '_terminal_layer_regression_kernel5_channel32.png'
